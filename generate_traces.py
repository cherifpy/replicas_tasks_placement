"""
Trace generator (infrastructure + workload) for evaluating the Heuristic approach of
"Cost-Aware Replicas Placement and Tasks Scheduling In Geo-Distributed Infrastructure"
(Si-Mohammed et al.).

Cleaned-up, self-contained version of the original notebook:
  simulator/workloads/workloads-100-storage-contrainte/job_genetation.ipynb

Reproduces the paper's experimental protocol (Section 5.1):
  - Infrastructure with 4 node categories, combining high/low bandwidth and compute
    power (~24/26/24/26% of nodes per category):
  ...
  - 4 instances from the paper: (10 jobs, 50 nodes), (20 jobs, 50 nodes), (20 jobs, 100 nodes),
    (50 jobs, 100 nodes).

Each instance is written into a subfolder traces/inst-{J}J-{N}N/ with two files:
  - infrastructure.csv: columns bandwidth,computation_nodes,energy_consumption,storage_capacity
    (format directly readable by simulator.generateHeterogeneousInfrastructureEquilibre)
  - jobs.json: list of jobs {nb_tasks, task_duration, dataset_size, id_dataset,
    arriving_time, type_of_job, job_id} (format directly readable by
    simulator.jobs_injector via config['jobs_file_path'])

Run: python3 generate_traces.py
"""
import json
import os
import random
...
# capacity (no notion of "a node too small for a dataset" in the paper's model -
# Section 4.2, utility + acceleration factor only). storage_capacity is therefore fixed
# to a value deliberately far above any possible dataset, so this filter (present in the
# engine for other uses of the repository, cf. SelectionnerMeilleurNoeud) is never
# binding here.
...
def generate_infrastructure(nb_nodes: int, seed: int = SEED) -> list[dict]:
    """Generates nb_nodes nodes spread across the 4 categories above. Returns a list
    of dicts {bandwidth, computation_nodes, energy_consumption, storage_capacity} - same
    ...
    for idx, cat in enumerate(categories):
        ...
        if idx == len(categories) - 1:
            count = nb_nodes - assigned  # the remainder, so the total always equals nb_nodes
        else:
            count = round(spec["share"] * nb_nodes)
            assigned += count
        for _ in range(count):
            # Draw order MATTERS (same random.* calls in the same order as the
            # original notebook, to stay bit-for-bit reproducible at a fixed seed):
            # vcpu, then bandwidth, then energy. No random draw for storage -
            # fixed, unlimited capacity (the paper does not model it, see above).
...
def generate_arrival_times(nb_jobs: int, lambda_rate: float, seed: int = SEED) -> list[float]:
    ...
    lambda_rate is, the heavier the load."""
    random.seed(seed)
    t = 0.0
    times = []
...
def generate_instance(nb_jobs: int, nb_nodes: int, lambda_rate: float,
    ...
    """Generates a complete instance (infrastructure.csv + jobs.json) in
